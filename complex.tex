...
\twocolumnsides{
  ...
  \item $ \abs{\abs{z_{1}}- \abs{z_{2}}} \leq \abs{z_{1} \pm z_{2}} \leq \abs{z_{1}} + 
      \abs{z_{2}} $
  \end{enumerate}
}
...
\begin{example}
  \[
    i^{27} = i^{4\cdot 6 +3} = (i^{4})^{6} \cdot i ^{3} = 1^{6} \cdot -i = -i \quad
    \text{ή πιο απλά} \quad i^{27} = i^{4\cdot 6+3} = i^{3} = -i
  \] 
\end{example}

\section*{Πολική μορφή Μιγαδικού αριθμού}

\InsertBoxL{4}{\parbox[b][4\baselineskip][c]{0.28\textwidth}{
    \begin{tikzpicture}
...
\textcolor{Col1}{εκθετική} μορφή ως:
\[
  z= re^{i\theta}
\]
...
\item {}
  \fourcolumnsides{
    \begin{tikzpicture}[scale=0.8]
      ...
      \draw[-stealth] (0,0) -- (0,2.2) node[left] {$y$} ;
      \coordinate (p) at (1.4,1.4) ;
      \fill[Col1] (p) node[above=5pt,font=\small,xshift=-5pt,Col2] {$z=1+i$} circle (1.5pt) ;
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      \fill pic[draw,fill=Col1!55,-latex,opacity=0.7,angle radius=11pt] {angle=x--0--p} ;
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      \fill (p) node[below=5pt,font=\small,xshift=-5pt,Col2] {$z=-1-i$} 
        circle (1.5pt) ;
      \draw (O) --  (p) node[left,Col1] {$M$} ;
      \fill pic[draw,fill=Col1!55,-latex,opacity=0.7,angle radius=11pt,latex-] {angle=p--0--x} ;
      \node[fill=Col2!25] at (-0.5,0.7) [left,draw,inner sep=2pt] {$\Arg= -3\pi /4$} ;
    \end{tikzpicture}
  }
  {
    \begin{tikzpicture}[scale=0.8]
      ...
      \node[fill=Col2!25] at (1.0,-0.5) [right,draw,inner sep=2pt] {$\Arg= -\pi /4$} ;
    \end{tikzpicture}
  }
\end{examples}
