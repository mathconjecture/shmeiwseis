...
\twocolumnsidesn{
  \section*{Κυλινδρικές Συντεταγμένες ($r$,$\theta$,$z$)}
  ...
  \begin{tikzpicture}[scale=0.8]
    ...
    \path pic[draw,scale=0.5] {right angle=p--p1--0};
  \end{tikzpicture}
  \threecolumnsidess{
  ...
  \section*{Σφαιρικές Συντεταγμένες ($\rho$, $\phi$, $\theta$)}
  \begin{tikzpicture}[scale=0.8]
    \coordinate (0) at (0, 20) ;
    \node at (0) [left,xshift=-5pt,yshift=4pt] {$0$} ;
    \coordinate (y) at (4, 20) ;
    ...
    \draw[dashed] (p1) -- (xp) node [left=3pt,yshift=2pt] {$x$} ;
    \draw[dashed] (p1) -- (yp) node [above right] {$y$};
    \draw[dashed] (p) -- (zp) node [left] {$z$} node[midway,above] {$r$};
    ...
    \draw (0.center) to node[pos=0.6,above] {$r$} (p1.center);
    \path pic[draw,scale=0.5] {right angle=p--p1--0};
    \path pic[draw,scale=0.5] {right angle=0--zp--p};
  \end{tikzpicture}
  \threecolumnsides{
    \begin{empheq}[box=\mathboxr]{gather*}
      x= \rho \sin{\phi} \cos{\theta} \\
